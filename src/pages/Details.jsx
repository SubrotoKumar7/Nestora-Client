...
const Details = () => {
  ...
  useEffect(() => {
    fetch(`https://nestora-server-api.vercel.app/reviews/${productId}`)
      .then((res) => res.json())
      .then((data) => {
        setReviews(data);
      });
  ...
  useEffect(() => {
    const calculateAverageRating = (reviews) => {
      const totalRating = reviews.reduce(
        (sum, review) => sum + review.productRating,
        0
      );
  ...
  const handleReviewSubmit = (e) => {
    ...
    fetch(`https://nestora-server-api.vercel.app/reviews`, {
      method: "POST",
      headers: {
    ...
    <div className="w-11/12 mx-auto sec-gap">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-7">
        ...
        <div className="space-y-2.5">
          ...
          <p>
            <strong>Property Build Year:</strong> {yearBuilt}
          </p>
          <p>
      ...
      <div className="my-10">
        ...
        <div className="mt-15">
          ...
          <div className="grid grid-cols-1 gap-7 mt-5">
            {reviews.map((data, i) => (
              <ReviewCard key={i} data={data}></ReviewCard>
            ))}
          </div>
        </div>
      </div>
